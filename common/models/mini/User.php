...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public function rules()
    {
        return [
			...
			[['status', 'r_id', 'created_at', 'last_login_date', 'integral', 'updated_at','token_add_time'], 'integer'],
            [['username'], 'required'],
			...
			['mobile', 'unique'],
			[['access_token','token'], 'string', 'max' => 100],
     ...
     * @return string
     */
    public static function createToken($model)
    {
        $model->token_add_time=time();
        $model->token = md5(base64_encode(md5($model->id) . md5(date('Y-m-d H:i:s')) .md5(SECRET_SALT)));
        if($model->save()){
            Yii::$app->cache->set(CacheKey::tokenUserId($model->token), $model,TOKEN_OUT_TIME);
            return $model->token;
        }
        return false;
    }
